...
import { Play, RotateCcw, ArrowLeft, Cpu, Gauge, Code2, Zap, Layers, TextQuote, Database } from 'lucide-react';
...
export default function LcsVisualizer() {
  ...
  const [status, setStatus] = useState('LCS Engine Ready');
  ...
  const PSEUDO_CODE = {
    python: { logic: "for i in range(1, m + 1):\n  for j in range(1, n + 1):\n    if S1[i-1] == S2[j-1]:\n      dp[i][j] = 1 + dp[i-1][j-1]\n    else:\n      dp[i][j] = max(dp[i-1][j], dp[i][j-1])", comp: "O(M * N) Complexity" },
    java: { logic: "if (S1.charAt(i-1) == S2.charAt(j-1))\n    dp[i][j] = 1 + dp[i-1][j-1];\nelse\n    dp[i][j] = Math.max(dp[i-1][j], dp[i][j-1]);", comp: "Bottom-Up Matrix" }
  };
  ...
  const startDP = async () => {
    ...
    setStatus("Phase 1: Initializing Base Cases (Row 0 & Col 0 = 0)");
    await delay();
    ...
    for (let i = 1; i <= m; i++) {
      for (let j = 1; j <= n; j++) {
        ...
        if (str1[i - 1] === str2[j - 1]) {
          ...
          setStatus(`Match! '${str1[i-1]}' == '${str2[j-1]}'. 1 + Diagonal(${table[i-1][j-1]})`);
        } else {
          table[i][j] = Math.max(table[i - 1][j], table[i][j - 1]);
          setStatus(`Mismatch. Max of Top(${table[i-1][j]}) and Left(${table[i][j-1]})`);
        }
    ...
    setCurrent({ i: -1, j: -1 });
    setIsRunning(false);
    setStatus(`Sequence Analysis Complete. LCS Length: ${table[m][n]}`);
  };
    ...
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 md:p-12 space-y-12 text-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/5 pb-10">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-5xl font-black tracking-tighter uppercase italic">LCS Visualizer</h1>
            <p className="text-gray-500 font-medium text-xs md:text-lg tracking-tight italic">Watch the matrix build the Longest Common Subsequence length bit by bit.</p>
          </div>
        </div>
      </div>
                  ...
                  {dp.map((row, i) => (
                    <tr key={i}>
                      ...
                      {row.map((val, j) => {
                        ...
                        const isMatch = i > 0 && j > 0 && str1[i-1] === str2[j-1] && isActive;
                        ...
                        return (
                          ...
                          } ${isMatch ? 'ring-2 ring-emerald-500 animate-pulse' : ''}`}>
                            {val}
                          </td>
                        );
                      })}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-10 bg-white/[0.03] border border-white/10 rounded-[2.5rem] backdrop-blur-xl shadow-xl">
            <h3 className="text-[11px] font-black text-gray-500 uppercase tracking-widest mb-10 flex items-center gap-3">
              <Database size={16} className="text-fuchsia-500" /> Matrix Input
            </h3>
            
            <div className="space-y-8">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">String A</label>
                  <input 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">String B</label>
                  <input 
              ...
              <div className="flex flex-col gap-4">
                <button onClick={startDP} disabled={isRunning} className="flex items-center justify-center gap-3 py-5 bg-fuchsia-600 rounded-2xl text-xs font-black tracking-widest text-white disabled:opacity-30 active:scale-95 transition-all shadow-xl shadow-fuchsia-900/20">
                  <Play size={18} fill="currentColor" /> POPULATE MATRIX
                </button>
