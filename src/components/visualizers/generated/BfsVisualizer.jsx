...
export default function BfsVisualizer() {
  ...
  const [status, setStatus] = useState('BFS Engine Ready');
  ...
  const startBfs = async () => {
    ...
    setQueue([...q]);
    setVisited(new Set(vSet));
    setStatus("Phase 1: Starting at Node 1. Adding to Queue.");
    await delay();
    ...
    while (q.length > 0) {
      ...
      setQueue([...q]);
      setActiveNode(u);
      setStatus(`Processing Node ${u}. Exploring adjacent neighbors...`);
      await delay();
      ...
      for (let neighbor of neighbors) {
        if (!vSet.has(neighbor)) {
          vSet.add(neighbor);
          q.push(neighbor);
          
          setVisited(new Set(vSet));
          setQueue([...q]);
          setStatus(`New Frontier: Discovered Node ${neighbor}. Appending to Queue.`);
          await delay();
        }
      }
    }
    ...
    setActiveNode(null);
    setStatus("Traversal Complete. All reachable nodes discovered.");
    setIsRunning(false);
  };
    ...
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 md:p-12 space-y-12 text-white selection:bg-fuchsia-500/30">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/5 pb-10">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-5xl font-black tracking-tighter uppercase italic">BFS Discovery</h1>
            <p className="text-gray-500 font-medium text-xs md:text-lg tracking-tight italic">Visualize level-order exploration and shortest-path discovery in unweighted graphs.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-3 flex flex-col items-center justify-center min-h-[600px] bg-[#0c0214] rounded-[3rem] p-8 border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 w-full flex flex-col items-center gap-3">
             <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Active Queue (FIFO)</span>
             <div className="flex gap-2 min-h-[50px] p-2 bg-white/5 border border-white/10 rounded-xl">
                {queue.length === 0 && <span className="text-gray-700 text-[10px] self-center px-4 italic">Queue Empty</span>}
                {queue.map((val, idx) => (
                  <div key={idx} className="w-10 h-10 flex items-center justify-center bg-fuchsia-600/20 border border-fuchsia-500 rounded-lg font-mono text-sm font-bold text-fuchsia-400 animate-in fade-in zoom-in">
                    {val}
                  </div>
                ))}
             </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-10 bg-white/[0.03] border border-white/10 rounded-[2.5rem] backdrop-blur-xl shadow-xl">
            <h3 className="text-[11px] font-black text-gray-500 uppercase tracking-widest mb-10 flex items-center gap-3">
              <Layers size={16} className="text-fuchsia-500" /> Frontier Controller
            </h3>
            ...
            <div className="space-y-10">
              ...
              <div className="flex flex-col gap-4">
                <button onClick={startBfs} disabled={isRunning} className="flex items-center justify-center gap-3 py-5 bg-fuchsia-600 rounded-2xl text-xs font-black tracking-widest text-white disabled:opacity-30 active:scale-95 transition-all shadow-xl shadow-fuchsia-900/20">
