...
import { Play, RotateCcw, ArrowLeft, Cpu, Gauge, Code2, Zap, Package, ShoppingBag, Database } from 'lucide-react';
...
export default function KnapsackVisualizer() {
  ...
  const [dp, setDp] = useState(Array(items.length + 1).fill(0).map(() => Array(CAPACITY + 1).fill(0)));
  const [current, setCurrent] = useState({ i: -1, w: -1 });
  const [isRunning, setIsRunning] = useState(false);
  const [status, setStatus] = useState('DP Engine Ready');
  ...
  const PSEUDO_CODE = {
    python: { logic: "for i in range(1, n + 1):\n  for w in range(1, W + 1):\n    if wt[i-1] <= w:\n      dp[i][w] = max(val[i-1] + dp[i-1][w-wt[i-1]], dp[i-1][w])\n    else:\n      dp[i][w] = dp[i-1][w]", comp: "O(n * W) Time/Space" },
    java: { logic: "for (int i = 1; i <= n; i++) {\n  for (int w = 1; w <= W; w++) {\n    if (wt[i-1] <= w)\n      dp[i][w] = Math.max(val[i-1] + dp[i-1][w-wt[i-1]], dp[i-1][w]);\n    else\n      dp[i][w] = dp[i-1][w];\n  }\n}", comp: "Bottom-Up Table Filling" }
  };

  const startDP = async () => {
    if (isRunning) return;
    setIsRunning(true);
    let table = Array(items.length + 1).fill(0).map(() => Array(CAPACITY + 1).fill(0));
    ...
    for (let i = 1; i <= items.length; i++) {
      ...
      for (let w = 1; w <= CAPACITY; w++) {
        ...
        if (item.wt <= w) {
          const includeVal = item.val + table[i - 1][w - item.wt];
          const excludeVal = table[i - 1][w];
          table[i][w] = Math.max(includeVal, excludeVal);
          setStatus(`Item ${item.name}: Comparing Include (${includeVal}) vs Exclude (${excludeVal})`);
        } else {
          table[i][w] = table[i - 1][w];
          setStatus(`Item ${item.name} too heavy for weight ${w}. Copying previous best.`);
        }
    ...
    setCurrent({ i: -1, w: -1 });
    setIsRunning(false);
    setStatus(`Optimization Complete. Max Value: ${table[items.length][CAPACITY]}`);
  };
    ...
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 md:p-12 space-y-12 text-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/5 pb-10">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-5xl font-black tracking-tighter uppercase italic">Knapsack DP</h1>
            <p className="text-gray-500 font-medium text-xs md:text-lg tracking-tight italic">Visualize the Bottom-Up table construction for the 0/1 Knapsack problem.</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 flex flex-col items-center justify-center min-h-[600px] bg-[#0c0214] rounded-[3.5rem] p-6 md:p-10 border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 w-full overflow-x-auto no-scrollbar">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="p-2 border border-white/10 text-[10px] text-gray-600 uppercase">Item \ Wt</th>
                  {[...Array(CAPACITY + 1)].map((_, w) => (
                    <th key={w} className="p-2 border border-white/10 text-[10px] text-gray-400 font-mono">{w}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {dp.map((row, i) => (
                  <tr key={i}>
                    <td className="p-2 border border-white/10 text-[10px] font-bold text-blue-400">
                      {i === 0 ? 'Base' : `${items[i-1].name} (v:${items[i-1].val}, w:${items[i-1].wt})`}
                    </td>
                    {row.map((val, w) => {
                      const isActive = current.i === i && current.w === w;
                      const isPrecalc = i < current.i || (i === current.i && w < current.w);
                      return (
                        <td key={w} className={`p-2 border border-white/10 text-center font-mono text-xs transition-all duration-300 ${
                          isActive ? 'bg-fuchsia-600 text-white scale-110 z-20 shadow-[0_0_15px_#d946ef88]' :
                          isPrecalc ? 'text-gray-300' : 'text-gray-700'
                        }`}>
                          {val}
                        </td>
                      );
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-10 bg-white/[0.03] border border-white/10 rounded-[2.5rem] backdrop-blur-xl shadow-xl">
            <h3 className="text-[11px] font-black text-gray-500 uppercase tracking-widest mb-10 flex items-center gap-3">
              <Database size={16} className="text-fuchsia-500" /> DP Controller
            </h3>
            ...
            <div className="space-y-10">
              ...
              <div className="flex flex-col gap-4">
                <button onClick={startDP} disabled={isRunning} className="flex items-center justify-center gap-3 py-5 bg-fuchsia-600 rounded-2xl text-xs font-black tracking-widest text-white disabled:opacity-30 active:scale-95 transition-all shadow-xl shadow-fuchsia-900/20">
                  <Play size={18} fill="currentColor" /> COMPUTE TABLE
                </button>
